...
func create_new_match(matchId: int, mapName: String, player_ids: Array[int]):
	...
	.stage("Loading map...", func(): battlefieldRenderer = renderGameBoard.instantiate(); add_child(battlefieldRenderer); Network.Match.update_client_match_state.rpc_id(1, MatchState.STATE.READY)) \
